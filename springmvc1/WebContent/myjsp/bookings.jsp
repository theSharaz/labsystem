  ...
  <ul class="nav nav-pills">
    ...
    <li class="noround"><a data-toggle="pill" href="#approved">Approved Times</a></li>
		...
		<form action="filterBook" method="GET" class="form-inline">
		  ...
		  <button type="submit" onclick="getBookings()" class="btn btn-primary mb-2">Search</button>
		</form>	
				...
				<c:forEach items="${sessionScope.aplist}" var="a">
          			<tr>
						...
						<td>${a.week}</td>
             			...
             			<td>${a.finish}</td>
             			<td>${a.room}</td>
             			<td><a href="approveApplication/${a.appid}" onclick="getBookings()">Accept</a>
             				<a href="rejectAppliction/${a.appid}" onclick="getBookings()">Reject</a></td>
           			</tr>
       			</c:forEach>
              </tbody>
      </table>
				...
				<c:forEach items="${sessionScope.atlist}" var="a">
          			<tr>
						...
						<td>${a.week}</td>
             			...
             			<td>${a.finish}</td>
             			<td>${a.room}</td>
             			<td><a href="removeApproved/${a.approvedid}" onclick="getBookings()">Remove</a></td>
           			</tr>
       			</c:forEach>
        </tbody>
      </table>
